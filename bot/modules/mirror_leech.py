...
async def _mirror_leech(client, message, isQbit=False, isLeech=False, sameDir=None, bulk=[]):
    ...
    if not is_url(link) and not is_magnet(link) and not await aiopath.exists(link) and not is_rclone_path(link) and file_ is None:
        btn = ButtonMaker()
        btn.ibutton('Cʟɪᴄᴋ Hᴇʀᴇ Tᴏ Rᴇᴀᴅ Mᴏʀᴇ ...', f'kpsmlx {message.from_user.id} help MIRROR', style='blue')
...
async def kpsmlxcb(_, query):
    ...
    if user_id != int(data[1]):
        ...
        try:
            ...
            endLine = "\n----------<b>END LOG</b>----------"
            btn = ButtonMaker()
            btn.ibutton('Cʟᴏsᴇ', f'kpsmlx {user_id} close', style='red')
        ...
        if data[3] == "CLONE":
            ...
            if len(data) == 4:
                ...
                btn.ibutton('Nᴇxᴛ Pᴀɢᴇ', f'kpsmlx {user_id} help MIRROR readmore', style='blue')
            else:
                msg = MIRROR_HELP_MESSAGE[1][4000:]
                btn.ibutton('Pʀᴇ Pᴀɢᴇ', f'kpsmlx {user_id} help MIRROR', style='blue')
            await editMessage(message, msg, btn.build_menu(2))
        if data[3] == "YT":
            await editMessage(message, YT_HELP_MESSAGE[1], btn.build_menu(1))
    elif data[2] == "guide":
        btn = ButtonMaker()
        btn.ibutton('Bᴀᴄᴋ', f'kpsmlx {user_id} guide home', style='blue')
        btn.ibutton('Cʟᴏsᴇ', f'kpsmlx {user_id} close', style='red')
        if data[3] == "basic":
            ...
            buttons = ButtonMaker()
            buttons.ibutton('Basic', f'kpsmlx {user_id} guide basic', style='blue')
            buttons.ibutton('Users', f'kpsmlx {user_id} guide users', style='blue')
            buttons.ibutton('Mics', f'kpsmlx {user_id} guide miscs', style='blue')
            buttons.ibutton('Owner & Sudos', f'kpsmlx {user_id} guide admin', style='blue')
            buttons.ibutton('Close', f'kpsmlx {user_id} close', style='red')
