...
async def confirm_category(client, query):
    ...
    if user_tds:
        for _name in user_tds.keys():
            buttons.ibutton(f'{"✅️" if cat_name == _name else ""} {_name}', f"scat {user_id} {msg_id} {_name.replace(' ', '_')}", style='green' if cat_name == _name else 'blue')
...
async def confirm_dump(client, query):
    ...
    if user_dumps:
        for _name in user_dumps.keys():
            buttons.ibutton(f'{"✅️" if upall or cat_name == _name else ""} {_name}', f"dcat {user_id} {msg_id} {_name.replace(' ', '_')}", style='green' if upall or cat_name == _name else 'blue')
